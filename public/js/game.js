...
class Board {
    ...
    movePiece(from, to) {
        // Make sure the move is from and to valid positions, and that a piece is moved
        if (Tile.valid(from) && Tile.valid(to) && this.getPiece(from)) {
            // Make sure the move is legal and it doesn't cause a check
            if (this.getLegalMoves(from).filter(move => move.equals(to)).length != 0 && this.pieceCanMove(from, to)) {
...
class Game {
    constructor(time, increment, color, founder) {
        this.time = time;
    ...
    attemptMove(message) {
        var moveSucessful = this.board.movePiece(new Position(message.from.x, message.from.y), new Position(message.to.x, message.to.y));
        if (moveSucessful)
            this.state = this.board.calculateGameState();
        return moveSucessful;
    }

    getCurrentPlayer() {
        return this.players[this.board.getCurrentPlayerColor()];
    }
